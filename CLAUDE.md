...
- Last session: 2026-07-31 — R1 (D-048): unified the three portals into ONE app + single /login (reverses 6.3; supersedes D-026/D-035). One merged router (src/router.tsx) with per-route RouteGuard→AccessDenied; deleted src/portals/* modules, config/portal.ts, WrongPortal; retired VITE_PORTAL (env/vite-env cleaned). RequireAuth = session → super_admin/owner AAL2 (MfaStepUp) → render. IndexRedirect + LoginPage land on homeRoute(role): super→/overview, cloud→/dashboard, onprem→/namespaces (onprem nav reordered Namespaces-first to keep home=first-nav). Verified local: 3 demo users sign in via same /login → correct home; ONE build; typecheck/lint/build green; no portal imports remain. Prior same day: 6.4b owner mgmt + owner sub-roles + Phase-0 lockdown + MFA hotfix (D-043–D-047).
