...
- Last session: 2026-07-31 — R2a (D-049–D-052): corrected on-prem hierarchy to cluster→namespace→org→tenant (mock-only, D-027). New types Cluster (first-class, decommissionable) + NamespaceOrg (org WITHIN a namespace); OnPremOrgDetail returns clusters:[{cluster,namespaces}]; tenants/ALL metrics re-scoped to nsOrg (fetchNamespaceOrgMetrics/…Orgs/…Org); NamespaceDetail slimmed to header+env vars. NamespaceClusters = per-cluster groups + cluster/namespace decommission (confirm modal → ephemeral status, D-009); NamespaceDetailPage = header+Upgrade/Decommission+Organizations list+Env Vars (metric tabs removed); NEW NamespaceOrgDetailPage = 5 DetailTabs at nested route /onprem-customers/:orgId/namespaces/:namespaceId/orgs/:nsOrgId. StatusBadge += decommissioned. Verified: nest via tsx, typecheck/lint/build green, dev serves 200. Prior: R1 single-login unify (D-048).
- Next up: R2b — feature-flag cluster scope (FlagScope += cluster; panel works at all 4 scopes; cluster+namespace "Edit feature flags" affordances; D-053). Then 6.5 live data layer.
