...
export function OnPremOrgDetailPage() {
  ...
  // Local, ephemeral namespace state (upgrade / add mutate this, not the mock
  // layer) — seeded once from the query, guarded against clobbering user edits
  ...
  function handleUpgradeConfirm() {
    ...
    setNamespaces((prev) =>
      (prev ?? []).map((n) => (n.id === upgradeTarget.id ? { ...n, version: latestVersion } : n))
    )
  ...
  return (
    <div className="space-y-6">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-1.5">
            ...
            <span className="text-sm text-muted-foreground">
              {list.length} namespace{list.length === 1 ? '' : 's'} across {clusters.length} cluster{clusters.length === 1 ? '' : 's'}
            </span>
          </div>
        </div>
        <button
      ...
      <div className="space-y-6">
        <h2 className="text-sm font-semibold text-foreground">Cluster &amp; Namespace Overview</h2>
        {clusters.length === 0 ? (
          <EmptyState title="No namespaces" description="This organization has no namespaces yet." />
        ) : (
      ...
      <Modal
        ...
        title={`Upgrade ${upgradeTarget?.name ?? ''}`}
      >
        {upgradeTarget && (
      ...
      {/* Edit env vars slide-over — real panel wired up in 5.7 */}
      <SlideOver
        open={!!envVarsTarget}
        onClose={() => setEnvVarsTarget(null)}
        title={`Environment variables — ${envVarsTarget?.name ?? ''}`}
      >
        <p className="text-sm text-muted-foreground">Environment variables editor — coming in 5.7</p>
      </SlideOver>
...
function AddNamespaceSlideOver({
  ...
  const canSubmit =
    ...
    (!isNewCluster || newClusterName.trim().length > 0)
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <select
            ...
            className="w-full rounded-md border border-border px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-primary/40"
          >
            {clusters.map((c) => (
        ...
        {isNewCluster && (
          <div>
            ...
            <input
              ...
              placeholder="frankfurt-prod-cluster"
              className="w-full rounded-md border border-border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/40"
            />
          </div>
        )}
        ...
        <div>
          ...
          <input
            ...
            className="w-full rounded-md border border-border px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-primary/40"
          />
        </div>

        <div className="flex justify-end gap-2 pt-2">
          <button type="button" onClick={onClose} className="rounded-md border border-border px-3 py-2 text-sm font-medium text-foreground hover:bg-gray-50 transition-colors">
            Cancel
          </button>
          <button
            type="submit"
